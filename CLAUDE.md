...
| `IMAGE_MODEL` | No | `black-forest-labs/FLUX.1-schnell` | HF text-to-image model id used by `/createimage`. Any HF text-to-image model works |
| `IMAGE_API_BASE` | No | `https://router.huggingface.co/hf-inference/models` | Base URL for the HF Inference API image call. `{IMAGE_API_BASE}/{IMAGE_MODEL}` is POSTed with `{"inputs": prompt}` |
...
- **`/createimage` needs `HF_TOKEN` set _on PA_, not just locally** — HF's hosted Inference API rejects anonymous calls, so without a token the command replies with a clean "couldn't create that image" apology (the real reason is logged, never shown). The token lives in `.env`, which is **untracked** — so editing your local `.env` does NOT update PA, and `/api/deploy`'s `git reset --hard` never touches it. You must edit `/home/<user>/<project>/.env` on PA directly (Files tab or a Bash console) and reload the web app. `router.huggingface.co` (the image endpoint host) IS on PA's free-tier whitelist, so the whitelist is not the blocker
- **`/createimage` cold start** — `generate_image()` sends `x-wait-for-model: true` and retries a transient 503 ("model is currently loading") within `IMAGE_REQUEST_TIMEOUT`. The first call after the model goes idle can take ~20s; subsequent calls are fast
