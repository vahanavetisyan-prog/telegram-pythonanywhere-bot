...
**Console output polling.** `pa_deploy.sh::run_remote` appends `; echo __PADEPLOY_DONE_<nanos>_<pid>__` to every command it sends, then polls `GET /consoles/<id>/get_latest_output/` every 3s until the marker appears (or it times out). Avoids blind `sleep` for slow operations like `pip install`.
