...
- **`/createimage` uses `router.huggingface.co`, not `huggingface.co`** — the whitelist entry is for `huggingface.co`; the Inference API routes through the `router.huggingface.co` subdomain. If `/createimage` works locally but hangs/403s on PA, verify with `curl -I https://router.huggingface.co/` from a PA Bash console and, if blocked, request it on the PA forum whitelist thread (same failure mode as `*.hf.space` for `/model hf`). `generate_image()` passes `timeout=IMAGE_REQUEST_TIMEOUT` so a blocked host fails fast instead of wedging the worker
- **`/createimage` needs `HF_TOKEN`** — HF's hosted Inference API rejects anonymous calls, so without a token the command replies with a clean "couldn't create that image" apology (the real reason is logged, never shown to the user)
