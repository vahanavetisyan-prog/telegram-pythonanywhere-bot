...
| `HF_TOKEN` | No | — | HF auth token. Needed if the Gradio space is private/gated, **and required for `/createimage`** (HF's hosted Inference API rejects anonymous calls) |
| `IMAGE_MODEL` | No | `black-forest-labs/FLUX.1-schnell` | HF text-to-image model id used by `/createimage`. Any model served by the providers in `IMAGE_PROVIDERS` works |
| `IMAGE_PROVIDERS` | No | `nscale,together` | Comma-separated, ordered list of HF Inference Providers to try for `IMAGE_MODEL`; first success wins. `hf-inference` is intentionally omitted — it's deprecating text-to-image models and was returning `500`/`410` (confirmed live 2026-07-08) |
| `IMAGE_API_BASE` | No | `https://router.huggingface.co` | Router root. `/createimage` POSTs `{IMAGE_API_BASE}/{provider}/v1/images/generations` (OpenAI-compatible) with `{"model", "prompt", "response_format": "b64_json"}` and base64-decodes `data[0].b64_json`. Returning the image *in the JSON body* is what keeps it on PA's whitelist — providers that reply with an image URL point at a CDN PA can't fetch |
| `IMAGE_REQUEST_TIMEOUT` | No | `55` | Wall-clock cap (seconds) on the image call. Kept under Telegram's ~60s webhook window since diffusion + cold starts are slow |
...
- **`/createimage` needs `HF_TOKEN` set _on PA_, not just locally** — the HF Inference Providers router rejects anonymous calls, so without a token the command replies with a clean "couldn't create that image" apology (the real reason is logged, never shown). The token lives in `.env`, which is **untracked** — so editing your local `.env` does NOT update PA, and `/api/deploy`'s `git reset --hard` never touches it. You must edit `/home/<user>/<project>/.env` on PA directly (Files tab or a Bash console) and reload the web app. `router.huggingface.co` (the image endpoint host) IS on PA's free-tier whitelist, so the whitelist is not the blocker. The token must be **fine-grained with the "Make calls to Inference Providers" permission** — a plain read token yields `403 "does not have sufficient permissions"` (confirmed 2026-07-08)
- **`/createimage` uses HF Inference *Providers*, not the old `hf-inference` backend** — `generate_image()` in `bot/providers.py` POSTs the OpenAI-compatible endpoint `{IMAGE_API_BASE}/{provider}/v1/images/generations` and base64-decodes `data[0].b64_json`. It tries each provider in `IMAGE_PROVIDERS` (default `nscale,together`) in order, sharing one wall-clock budget; a `model:`/`provider:`/`network:`-prefixed error rolls to the next provider, while `auth:`/`billing:` errors abort immediately (no other provider fixes a bad token or empty credit balance). **Do not point this back at `hf-inference`** — as of 2026-07-08 it returns `500` for FLUX.1-schnell and `410 deprecated` for SDXL, and only two text-to-image models remain mapped to it. A quick PA-side probe: `curl -s -X POST "https://router.huggingface.co/nscale/v1/images/generations" -H "Authorization: Bearer $HF_TOKEN" -d '{"model":"black-forest-labs/FLUX.1-schnell","prompt":"a cat","response_format":"b64_json"}'` should return `200` with a `b64_json` field
- **`/createimage` free-credit ceiling (`402`)** — HF Inference Providers bills per image against a small free monthly credit balance. When it's exhausted the providers return `402`; `generate_image()` surfaces a `billing:`-prefixed error and does not fall through (every provider shares the same balance). Fix is to enable HF billing or wait for the monthly reset
